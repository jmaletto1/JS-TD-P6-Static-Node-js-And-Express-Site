...
app.use((req, res, next) => {
  const err = new Error(
    "This is not the page you're looking for! Please visit our homepage, and try again."
  );
  err.status = 404;
  next(err);
});
...
app.use((err, req, res, next) => {
  ...
  if (err.status === 404) {
    console.log(
      "Unfortunately this link is not valid! Please return to the homepage."
    );
    res.locals.message = "Not found!";
    res.status(404).render("page-not-found", { err });
  } else {
    console.log(
      "Unfortunately there seems to have been a server error! Please return to the homepage."
    );
